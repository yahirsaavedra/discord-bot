...
def is_command():
    """
    Checks if CENSURADO is a bash command.
...
def update_CENSURADO():
    """
    Updates CENSURADO via Git.
    ...
    if get_latest_release() == __version__:
        info("CENSURADO is already the latest version."
        ...
        try:
            ...
            return
        if not code:
            info("\nCENSURADO is now updated successfully!")
...
def reset_CENSURADO(reqs=False, data=False, cogs=False, git_reset=False):
    """
    Resets CENSURADO or any of its properties (enabled to True boolean).
    If all of the parameters are enabled, CENSURADO will get a 'factory reset'.
    ...
    if git_reset:
        ...
        if code == 0:
            info(
                "CENSURADO repaired successfully! to the last local commit. "
...
def open_docs():
    ...
    """
    webbrowser.open("http://CENSURADObot.xyz/docs")
...
def download_ffmpeg(bitness):
    ...
    """
    clear_screen()
    repo = "https://github.com/CENSURADObot/CENSURADObot"
    verified = []
...
def maintenance_menu():
    ...
    while True:
        ...
        if choice == "1":
            ...
            if user_pick_yes_no():
                reset_CENSURADO(git_reset=True)
                pause()
        elif choice == "2":
            ...
            if user_pick_yes_no():
                reset_CENSURADO(data=True)
                pause()
        elif choice == "3":
            reset_CENSURADO(reqs=True)
            pause()
        elif choice == "4":
            ...
            if user_pick_yes_no():
                reset_CENSURADO(reqs=True, data=True, cogs=True, git_reset=True)
                pause()
        elif choice == "0":
            break
        clear_screen()
...
def run_CENSURADO(autorestart):
    """
    Start CENSURADO, autorestart is toggleable.
    ...
    if not run_script.is_file():
        error(
            "CENSURADO's main file to run is not available. "
            "Please reinstall CENSURADO!")
        pause()
        main()
    try:
        if autorestart:
            cmd = ("pm2", "start", "run.py", "--name=CENSURADO",
                   "--interpreter=" + interpreter)
            code = subprocess.call(cmd)
        else:
            cmd = (interpreter, "run.py")
            info("CENSURADO is now started as single session! "
                 "To shutdown the bot, press CTRL+Z anytime.")
            code = subprocess.call(cmd)
    except KeyboardInterrupt:  # Triggered!
        code = 0
    if not autorestart and code is 0:  # If no error
        info("CENSURADO has been terminated recently. Exit code: %d" % code)
    elif not autorestart and code is not 0:  # If error
        error("CENSURADO has been terminated recently. Exit code: %d" % code)
    elif autorestart and code is not 0:
        clear_screen()
        info("CENSURADO is already started! Restarting instead...")
        cmd = ("pm2", "restart", "run.py")
        subprocess.call(cmd)
    elif autorestart and code is 0:
        info("CENSURADO has been started successfully!")
    else:  # If error
        error("CENSURADO has been terminated recently. Exit code: %d" % code)
    pause()
...
def stop_CENSURADO():
    """
    Stops CENSURADO from running if started with PM2
    :return: Exit code, 0 if fine, else more than 0.
    """
    interpreter = sys.executable.split('/')[-1]
    if not interpreter:
    ...
    if code is 0:
        info("CENSURADO has been terminated successfully.")
    else:  # If error
        error("CENSURADO is not registered into PM2. No need to use this now.")
    pause()
...
def about():
    ...
    """
    try:
        print("CENSURADO ~The Discord bot~\n\n"
              "Developers: {}\nVersion: {}".format(', '.join(__author__),
                                                   BOT_VERSION),
              "\nHelpers: {}\n\n"
              "Website: http://CENSURADObot.xyz\n"
              "Twitter: @CENSURADOBot\n\n".format(
...
def edit_settings():
    ...
    if path.is_file():
        ...
        info("It looks like it's your first time running CENSURADO launcher.\n"
        ...
        error(
            ...
            "please reinstall CENSURADO from zero.\n")
        pause()
...
def detect_errors():
    ...
    or terminate immediately the launcher or CENSURADO's process.
...
def string_errors():
    ...
    if not is_git():
        warning("This installation was not done via Git\n"
                ...
                "source, please reinstall CENSURADO"
                "as it follows in the guide. "
                "http://www.CENSURADObot.xyz/docs\n\n")
    ...
    if not verify_requirements():
        ...
        error(
            ...
            "you won't be able to run CENSURADO. "
    ...
    if not admin_running():
        ...
        warning("Process is not running as administrator. Administrator "
                ...
                "permissions are disabled. Please restart CENSURADO by " + note +
                ".\n\n")

        
def bash_command():
    with open("/root/.bashrc", "w") as f:
        f.write("alias CENSURADO='" + sys.path[0] + "/" + sys.argv[0] + "'")
        f.close()
...
def main():
    ...
    while True:
        ...
        print("Bot options:")
        print("1. Start CENSURADO with autorestart")
        print("2. Stop CENSURADO if started with autorestart")
        print("3. Start CENSURADO with no autorestart\n")
        print("Core options:")
        ...
        choice = user_choice()
        if choice == "1":
            run_CENSURADO(autorestart=True)
        elif choice == "2":
            stop_CENSURADO()
        elif choice == "3":
            run_CENSURADO(autorestart=False)
        elif choice == "4":
            run_unittest()
        elif choice == "5":
            update_menu()
...
def run():
    ...
    if not SYSTEM_OK:
        error("Sorry! This operation system is not compatible with "
              "CENSURADO's environment and might not run at all. CENSURADO "
              "is only supported for Windows, Mac, Linux and "
              "Raspberry Pi. Please install one of those OS and try "
              "again.")
        exit(1)
    elif not PYTHON_OK:
        error("Sorry! This Python version is not compatible. CENSURADO needs "
